...
class AppStore extends Component {
  state = {tabItem: tabsList[0].tabId, value: ''}

  updateState = tabName => {
    this.setState({tabItem: tabName})
  }

  getList = () => {
    const {tabItem, value} = this.state
    const l = appsList.filter(x => x.category === tabItem)
    const listI = l.filter(x =>
      x.appName.toLowerCase().includes(value.toLowerCase()),
    )
    return listI
  }
        ...
        <div className="contain">
            ...
            <ul className="hel">
              {tabsList.map(x => (
                <TabItem
                  key={x.tabId}
                  val={x}
                  tabUpdateFunction={this.updateState}
                  isActive={x.tabId === tabItem}
                />
              ))}
            </ul>
          </div>
          <div className="apps-box">
            <ul className="app-container">
              {lis.map(x => (
                <AppItem key={x.appId} val={x} tab={tabItem} />
              ))}
            </ul>
          </div>
        </div>
